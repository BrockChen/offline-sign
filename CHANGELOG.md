...
- SetupVC 重构为 imToken 式布局（配色维持深色主题）：**保留品牌横排**（logo + wordmark + 副标题，首屏不苍白）+
  ...
  叠加 placeholder「输入助记词单词，用空格分隔」）+ keystore 口令框 + **底部固定主按钮**。
- **主按钮启用态**：助记词与口令均非空才启用（橙色），否则禁用（灰）。
- **键盘规避**：键盘弹出时底部按钮上移不被遮挡。
- 验证：typecheck 双分支 + Debug/Release BUILD SUCCEEDED；模拟器截图确认空态（placeholder + 按钮禁用）与有内容态（按钮启用）。
